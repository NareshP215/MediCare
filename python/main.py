...
def helper(disease):
    desc = " ".join(
        description[description["Disease"] == disease]["Description"].values
    )

    pre = precautions[precautions["Disease"] == disease][
        ["Precaution_1", "Precaution_2", "Precaution_3", "Precaution_4"]
    ].values.flatten()
    pre = [p for p in pre if str(p).strip() and str(p).lower() != "nan"]

    meds = medications[medications["Disease"] == disease][
        ["Medication1", "Medication2", "Medication3", "Medication4", "Medication5"]
    ].values.flatten()
    meds = [m for m in meds if str(m).strip() and str(m).lower() != "nan"]

    diets_list = diets[diets["Disease"] == disease][
        ["Diet1", "Diet2", "Diet3", "Diet4", "Diet5"]
    ].values.flatten()
    diets_list = [d for d in diets_list if str(d).strip() and str(d).lower() != "nan"]
...
def predict_json():
    ...
    dis_des, precautions_list, medications_list, diet_list, workout_list = helper(
        predicted_disease
    )
...
def measure_ecg():
    ...
    return jsonify(
        {
            "samples_collected": len(ecg_values),
            "average_ecg_value": avg_val,
            "estimated_bpm": bpm,
            "status": status,
            "all_values": ecg_values,
        }
    )
